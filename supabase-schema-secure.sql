-- RTS Funnel Enhanced Supabase Database Schema (SECURE)
-- Run this in your Supabase SQL Editor to support the enhanced commission system
-- This version includes enterprise-grade security without permission issues
...
CREATE TABLE IF NOT EXISTS leads (
  ...
  -- Metadata for AI Training (SECURE)
...
ALTER TABLE bonus_flags ENABLE ROW LEVEL SECURITY;

-- SECURITY POLICY: Agents can only view their own data
CREATE POLICY "Agents can view their own data" ON agents
  FOR SELECT USING (code = auth.jwt() ->> 'agent_code');

-- SECURITY POLICY: Public can insert leads (for forms)
CREATE POLICY "Public can insert leads" ON leads
  FOR INSERT WITH CHECK (true);

-- SECURITY POLICY: Agents can only view their own leads
CREATE POLICY "Agents can view their own leads" ON leads
  FOR SELECT USING (agent_id IN (
    SELECT id FROM agents WHERE code = auth.jwt() ->> 'agent_code'
  ));

-- SECURITY POLICY: Admins can view all leads
CREATE POLICY "Admins can view all leads" ON leads
  FOR SELECT USING (auth.jwt() ->> 'role' = 'admin');

-- SECURITY POLICY: Admins can update all leads
CREATE POLICY "Admins can update all leads" ON leads
  FOR UPDATE USING (auth.jwt() ->> 'role' = 'admin');

-- SECURITY POLICY: Public can view active QR codes
CREATE POLICY "Public can view active QR codes" ON qr_codes
  FOR SELECT USING (active = TRUE);

-- SECURITY POLICY: Admins can manage all QR codes
CREATE POLICY "Admins can manage all QR codes" ON qr_codes
  FOR ALL USING (auth.jwt() ->> 'role' = 'admin');

-- SECURITY POLICY: Public can insert bonus flags
CREATE POLICY "Public can insert bonus flags" ON bonus_flags
  FOR INSERT WITH CHECK (true);

-- SECURITY POLICY: Agents can only view their own bonus flags
CREATE POLICY "Agents can view their own bonus flags" ON bonus_flags
  FOR SELECT USING (agent_id IN (
    SELECT id FROM agents WHERE code = auth.jwt() ->> 'agent_code'
  ));

-- SECURITY POLICY: Admins can manage all bonus flags
CREATE POLICY "Admins can manage all bonus flags" ON bonus_flags
  FOR ALL USING (auth.jwt() ->> 'role' = 'admin');

-- Insert default agent (AHRPE5559) - SECURE
...
-- Create function to update updated_at timestamp (SECURE VERSION)
...
-- Create function to calculate commission breakdown (SECURE VERSION)
