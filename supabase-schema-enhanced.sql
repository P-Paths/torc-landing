-- RTS Funnel Enhanced Supabase Database Schema
-- Run this in your Supabase SQL Editor to support the enhanced commission system

-- Enable Row Level Security (Supabase handles JWT automatically)

-- Create agents table (enhanced for commission tracking)
CREATE TABLE IF NOT EXISTS agents (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  code VARCHAR(50) UNIQUE NOT NULL, -- e.g., AHRPE5559 (changed from agent_id)
...
CREATE TABLE IF NOT EXISTS leads (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  agent_id UUID REFERENCES agents(id), -- FK to agents.id (changed from agent_id VARCHAR)
  ...
  -- Contact Information
  first_name VARCHAR(255), -- Changed from gamer_first_name
  last_name VARCHAR(255),  -- Changed from gamer_last_name
  email VARCHAR(255),
  phone VARCHAR(20),
  address TEXT,
  city VARCHAR(100),
...
CREATE TABLE IF NOT EXISTS bonus_flags (
  ...
  agent_id UUID REFERENCES agents(id), -- Changed from agent_id VARCHAR
...
-- RLS Policies for bonus_flags table
DO $$ BEGIN
